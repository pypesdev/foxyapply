...
func getDBPath() (string, error) {
	...
	switch runtime.GOOS {
	case "darwin":
		// macOS: ~/Library/Application Support/DesktopApply
		home, err := os.UserHomeDir()
		if err != nil {
			return "", err
		}
		baseDir = filepath.Join(home, "Library", "Application Support", "DesktopApply")

	case "windows":
		// Windows: %APPDATA%\DesktopApply
		...
		baseDir = filepath.Join(appData, "DesktopApply")

	default:
		// Linux and others: ~/.config/applyfox
		...
		baseDir = filepath.Join(configDir, "applyfox")
	}
...
func (s *Store) migrate() error {
	migrations := []string{
		...
		// Migration 2: Create notes table (hello world example)
		...
		// Migration 5: Add new columns to linkedin_profiles
